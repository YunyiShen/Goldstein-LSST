...
def simulate_lsstLC_QC(Flambda, time, # time in days
                    wavelength, 
    ...
    ###### LSST schedule ######
    if LSSTschedule is not None:
        LSSTschedule = pd.read_pickle(LSSTschedule)
    else:
        with resources.path("package.data").joinpath("baseline_v3.3_10yrs.pkl") as path:
            LSSTschedule = pd.read_pickle(path)
    ...
    for _ in range(max_retry_location):
        ...
        Flambda = ori_Flambda
        time = ori_spec_time
        wavelength = ori_spec_wavelengths
        ...
        #resample the SED surface to the times of the observations
            ...
            for flt in filters:
                if filters_loc is None:
                    with resources.path("package.data").joinpath(f"filters/LSST_LSST.{flt}.dat") as path:
        ...
        # Convert counts to numpy arrays
            ...
            for flt in all_filters:
                ...
                phototime_tmp[:len_this_band] = time[flt_idx]/86400
            ...
            if np.sum(photomask) > minimum_LC_size:
                return photoband, photomag, phototime, photomask
...
def plot_lsst_lc(photoband, photomag, phototime, photoerror, photomask):
